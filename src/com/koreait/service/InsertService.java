...
//	singleton pattern은 프로그램에서 한순간에 단 한개의 객체만 필요할 경우 사용하는 클래스 디자인 패턴이다.
//	singleton pattern 코딩 방법 3단계
//	1. 자기자신의 객체를 기본 생성자로 정적멤버로 만든다.
...
//	insertOK.jsp에서 넘어오는 테이블에 저장할 데이터가 저장된 객체를 넘겨받고 mapper를 얻어오는 메소드
	public void insert(GuestbookVO vo) {
		System.out.println("InsertService 클래스 insert() 메소드 실행");
//		System.out.println(vo);
		
//		mapper를 얻어온다.
//		mapper에는 데이터베이스에 연결하는 connection과 연결된 후 실행할 sql명령(guestbook.xml)이 저장되어 있다. 
