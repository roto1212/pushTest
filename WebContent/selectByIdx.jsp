...
<body>
	<!-- 수정 또는 삭제할 글 1건을 얻어와서 request 영역에 저장한 후 수정 또는 삭제할 글을 브라우저에 표시하는 표시로 넘겨준다. -->
	...
	<%
		...
		//수정 또는 삭제할 글이 없으면 에러메시지를 출력하고 list.jsp로 돌려보내고 글이 테이블에 존재하면 수정 버튼이 클릭된 경우 수정할 글을 브라우저에 표시하는 페이지(update.jsp)로, 삭제 버튼이 클릭되면 삭제할 글을 브라우저에 표시하는 페이지(delete.jsp)로 넘겨준다.
		if (vo == null) {
			// 수정 또는 삭젤할 글이 테이블에 존재하지 않을 때
			out.println("<script>");
			out.println("alert('"+idx+"번글은 존재하지 않습니다.')");
